...
class TrainingEnvironment(Env, ABC):
    def __init__(self, game_process):
        ...
        self.comms.send_message(header=Message.RLGYM_CONFIG_MESSAGE_HEADER, body=list({'team_size': 1, 'self_play': 0, 'spawn_opponents': 0, 'tick_skip': 8, 'game_speed': 100}.values()))
    ...
    def reset(self):
        ...
        self.previous_observation = self.learning_object.get_observation(current_state)
    ...
    def step(self, raw_actions: Any):
        ...
        reward = self.learning_object.get_reward(previous_observation, actions, current_observation)
