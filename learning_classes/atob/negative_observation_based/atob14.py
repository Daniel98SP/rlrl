...
class ATOB14:
    ...
    def get_observation(self, state: GameState):
        ...
        observation = dict()
        observation['angle_to_point'] = [calcs.get_velocity_angle_to_point(state.players[0].car_data.position, state.players[0].car_data.linear_velocity, self.point_position)]
    ...
    def get_reward(previous_obs, actions, current_obs):
        ...
        reward += - np.log10(np.abs(current_obs['angle_to_point'][0]) * 10)
