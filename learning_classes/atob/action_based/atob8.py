...
class ATOB8:
    ...
    def get_spaces():
        ...
        observation_space_dictionary['rotation_angle_to_ball'] = spaces.Box(low=-180, high=180, shape=(1,), dtype=np.float16)
    ...
    def get_observation(self, state: GameState):
        ...
        observation = dict()
        observation['rotation_angle_to_ball'] = [calcs.get_yaw_angle_to_point(state, self.point_position)]
    ...
    def get_reward(previous_obs, actions, current_obs):
        ...
        if -0.5 < current_obs['rotation_angle_to_ball'][0] < 0.5:
            reward += 10
            reward += actions['throttle'] * 5
            reward += - np.abs(actions['steer']) * 5

        elif (previous_obs['rotation_angle_to_ball'][0] < 0 and actions['steer'] > 0 and actions['throttle'] > 0) or \
                (previous_obs['rotation_angle_to_ball'][0] > 0 and actions['steer'] < 0 and actions['throttle'] > 0):
            reward += 1
        else:
            reward += -1
