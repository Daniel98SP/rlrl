...
class ATOB12:
    ...
    def get_spaces():
        ...
        observation_space_dictionary['distance_to_ball'] = spaces.Box(low=0.0, high=20000, shape=(1,), dtype=np.float32)
    ...
    def get_observation(self, state: GameState):
        ...
        observation['distance_to_ball'] = [calcs.get_2d_distance(state.players[0].car_data.position, self.point_position)]

        return observation

    @staticmethod
    def get_reward(previous_obs, actions, current_obs):
        reward = 1000 if current_obs['distance_to_ball'][0] <= 200 else 0

        reward += 10 / np.abs(current_obs['angle_to_ball'][0])

        if -2 < current_obs['angle_to_ball'][0] < 2:
            reward += 2000 / current_obs['distance_to_ball'][0]
